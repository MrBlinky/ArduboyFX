/* *****************************************************************************
 * FX drawFrame example v1.00 by Mr.Blinky    Jan 2023 licenced under CC0
 * *****************************************************************************
 * 
 * This is an example that shows how you can draw animations from the FX
 * external flash memory using the drawFrame functions
 * 
 ...
 * 
 * Alternatively the fxdata.txt script file can be build using the
 * fxdata-build.py  Python script. using the command:
 *
 ...
 *
 * On some OS you may need to type python3 in front of the script name and
 * supply the full path to the script and fxdata files.
 * 
 ******************************************************************************
 *
 ...
 * -------------------------------
 * This function is a bit like FX::drawBitmap but instead of passing the
 * parameters along with the function directly, they are stored in FX memory at
 * the given address. In addition to that, parameters of multiple bitmaps can
 * be stored at the address to draw multiple bitmaps by at once.
 ...
 * can be added to mark the end of a frame or to mark the frame as the last
 * frame in the list of frames.
 *
 * This functions returns the address of the next frame unless the frame is
 ...
 *   Each frame is drawn for repeat + 1 times.
 *
 * uint8_t  FX::drawFrame()
 * ------------------------
 *
 ...
 * drawn and it is the last repeated frame, the current frame pointer is
 * reset to start of the frame list. This effectively loops the frame list.
 * The function also returns 0 in that case. This can be used to detect the 
 * end of the frame list animation.
 *
 ******************************************************************************/
...
void setup() {
  ...
  //arduboy.setFrameRate(60);    // Only needed for frameRates other than 60 fps
    ...
    case State::logo:
      {                          // draw a frame from the frame list and test if it was the last frame
        if(!FX::drawFrame()) {   // without this test the animation would loop continiously
            state = State::wait; // it was the last frame so change to wait state
